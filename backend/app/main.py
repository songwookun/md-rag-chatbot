...
async def _warm_reranker() -> None:
    ...
    ★ 왜 실제와 같은 크기로 부르는가
      ...
      작은 입력으로 데우면 첫 실제 질문이 9.9초 걸렸다(안정 상태 2.2초). 실측:
          1회차 9.86 → 2회차 4.01 → 5회차 2.76 → 안정 2.2초

      한때 이걸 "MPS 가 텐서 모양마다 재컴파일한다"로 설명했는데 **틀렸다.**
      나중에 재보니 배치 1~5개를 전부 데워도, 패딩을 512 로 고정해도 달라지지
      않았고, 재컴파일을 하지 않는 CPU 도 같은 양상이었다. 실재하는 건
      **첫 호출 1회 초기화 비용**뿐이다 — 다만 그 초기화를 제대로 시키려면
      실제와 비슷한 크기를 한 번 통과시켜야 한다.
